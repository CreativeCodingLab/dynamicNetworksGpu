...
*/
float* getData(info imagesInfo){

	std::cout << "Loading data from file\n";

	/// Allocate the meomory in RAM.
	float *data = (float*)malloc(imagesInfo.pixelNumber * imagesInfo.imageNumber * sizeof(float));
	log("Memory needed for the data: " + std::to_string(imagesInfo.pixelNumber * imagesInfo.imageNumber * sizeof(float) / 1000000) + " MB");
